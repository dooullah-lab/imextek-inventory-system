...
import { LayoutGrid, BarChart3, History, LogOut, Menu, X, ShieldCheck } from "lucide-react";
import { useState } from "react";

const baseNavItems = [
  { to: "/", label: "Inventory", icon: LayoutGrid, end: true },
...
export default function AppLayout() {
  ...
  const navItems = user?.role === "admin"
    ? [...baseNavItems, { to: "/users", label: "Users & Roles", icon: ShieldCheck }]
    : baseNavItems;
  ...
  const SidebarContent = () => (
    <>
      ...
      <div className="px-3 pb-5 pt-3 border-t border-brand-50">
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 md:hidden">
          ...
          <aside className="absolute left-0 top-0 h-full w-64 bg-white flex flex-col shadow-xl">
            <button
              onClick={() => setMobileOpen(false)}
              className="absolute right-3 top-5 text-brand-400"
            >
              <X size={20} />
            </button>
            <SidebarContent />
